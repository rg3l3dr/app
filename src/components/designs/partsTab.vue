        ...
        <table class="ui very basic small compact table" id='bomTable' v-if='parts.length'>
          ...
          <tbody name='fade' is="transition-group">
            <tr v-for='(part, index) in parts' :key='part.design_number'>
              ...
              <td class='collapsing'>
                <a href="#">
                  <i v-if='part.parts' class="cubes icon"></i>
              ...
              <td v-else-if='part.created && isEditable(index) && $route.params.revision_slug == "latest" '>
              ...
              <!-- Part # -->
              <td id='part-number' v-if='revision.name == "latest"'>
                <div class="ui dropdown refs" v-if='part.created'>
                  ...
                  <div class="menu">
                    <div class="item" v-for='revision in part.revisions' @click='setTrackingRef(revision, index, part)'>
                      {{ revision.name }}
                    </div>
                  </div>
                </div>
              ...
              <td
                v-else-if='part.created && isEditable(index) && $route.params.revision_slug == "latest"'
                class='collapsing'
              >
                <div class="ui transparent input" style='width:50px'>
                  <input
              ...
              <!-- Part Cost -->
              <td v-if='!part.created && $route.params.revision_slug == "latest"' class='collapsing'>
                <div class="ui transparent input" style='width:50px'>
                  <input
                    type="number"
              ...
              <td v-else-if='part.created && isEditable(index) && $route.params.revision_slug == "latest" && part.parts.length == 0' class='collapsing' >
                <div class="ui transparent input" style='width:50px'>
                  <input
                    id='part-cost-editable'
                    type="number"
...
export default {
  ...
  methods: {
    ...
    getSearchResult(index) {
      ...
      setTimeout(() => {
        if (vue.resultSelected) {
          ...
          vue.checkIfInBOM(index, vue.result.slug, vue.result.owner)
        } else {
          if (vue.env != 'prod') {
          ...
          if (vue.newPartName.data.trim()) {
            ...
            vue.$http.get('designs/' + vue.name_slug + '/').then(success => {
              ...
              vue.checkIfInBOM(index, vue.name_slug, vue.result.owner)
            }, error => {
    ...
    checkIfInBOM(index, result, owner) {
      ...
      if (part_slugs.includes(result)) {
        ...
        if (matched_part.ref_slug != 'alpha') {
          ...
          this.addExistingDesign(index, result, owner)
        } else {
          if (this.env != 'prod') {
    ...
    addExistingDesign: async function(index, result, owner) {
    ...
    openPart(index) {
      ...
      if (selectedPart.ref_type == 'config') {
        ...
        this.$route.params.revision_slug = 'latest'
        this.$route.params.build_slug = null
      } else if (selectedPart.ref_type == 'rev') {
        this.$route.params.config_slug = selectedPart.config_slug
        this.$route.params.revision_slug = selectedPart.ref_slug
        this.$route.params.build_slug = null
      } else if (selectedPart.ref_type == 'build') {
        this.$route.params.config_slug = selectedPart.config_slug
        this.$route.params.revision_slug = null
  ...
  created() {
    ...
    if (this.design) {
      ...
      this.$store.dispatch('getParts', payload)
